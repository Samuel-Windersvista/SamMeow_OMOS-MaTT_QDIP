...
- 启动/更新脚本：`启动.bat`（便携环境启动器 + 首次引导调度）、`更新组件.bat`（增量更新 opencode）
...
- 首次引导服务源码：`setup/`（见 `setup/codemap.md`，启动器与配置写入器的消费方）
- 终端用户文档：`玩家使用指南.md`（新手 5 步上手与工作流建议）、`验证清单.md`（QA 用 FAQ 对照表）

## Design

- **便携应用模式（Portable App）**：`启动.bat` 通过 `PATH`/`XDG_DATA_HOME`/`XDG_CONFIG_HOME` 环境变量把 Node 运行时、opencode CLI、认证目录、配置目录全部注入/重定向到包内路径（`runtime\node`、`opencode\bin`、`opencode\auth`、`opencode\config`），不依赖系统安装与用户全局配置。
...
- **引导调度器（Bootstrap Orchestrator）**：`启动.bat` 是首次引导的调度中枢——先启动后台 HTTP 引导服务，轮询等待 `.guide-url` 出现（上限 15 次、每次 `ping` 间隔约 1 秒），再用 `start` 调系统默认浏览器打开引导页，随后退出让玩家自行重开。
- **配置模板 + 运行期填充（Template + Runtime Injection）**：`opencode.json` 的 `provider` 段初始为空、`oh-my-opencode-slim.json` 携带默认模型分配，二者均作为模板由 `setup/config-writer.js` 在玩家保存配置时原地改写。注意 opencode 配置键为单数 `provider`（1.18.27 起复数 `providers` 会被诊断为 unsupported 并静默丢弃）。
- **后台子代理开关**：`启动.bat`/`进入环境.bat` 注入 `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true`（OMOslim 编排器官方安装器同款要求，缺失时 ping all agents 等编排能力不可用）。
...
```
玩家双击 启动.bat
  → 设置 ROOT/PATH/XDG_*/ROOTS 环境变量（ROOTS 为去尾反斜杠形态，供 guide-server 接收）
...
**已配置启动（正常路径）**：`启动.bat` → 环境注入 → `opencode %*` → 退出码透传。
...
**构建部署（build.ps1 → templates/）**：`启动.bat`/`更新组件.bat`/`验证清单.md`/`玩家使用指南.md` 复制到 outDir 根；`setup\*` 复制到 `outDir\setup`；`opencode.json` 与 `oh-my-opencode-slim.json` 复制到 `outDir\opencode\config\opencode\`（opencode 在 XDG_CONFIG_HOME 下自动追加 `opencode` 段，实际读取该目录）。

## Integration

- **消费方（被谁调用/读取）**：
  - `启动.bat`：被玩家双击；内部调用 `runtime\node\node.exe setup\guide-server.js`、`opencode` CLI、`start` 打开浏览器；读取 `data\.configured` 与 `data\.guide-url`。参数 `--check` 进入自检模式。
  ...
  - `玩家使用指南.md`/`验证清单.md`：被 build.ps1 复制进分发 zip，供玩家阅读。
